...
if (isset($conn) && $conn) {
    $query = "
        SELECT 
            ...
            u_customer.phone_no as customer_phone,
...
<head>
  ...
  <title>All Appointments | TrimBook Admin</title>
...
<body class="bg-black text-white antialiased">
  ...
  <aside id="sidebar" class="sidebar fixed left-0 top-0 h-full w-80 bg-gradient-to-b from-gray-900 to-gray-950 border-r border-gray-800 z-50 overflow-y-auto">
    <div class="p-6">
      ...
      <nav class="space-y-2">
        ...
        <a href="../dashboards/admin_allAppointment.php" class="flex items-center space-x-3 px-4 py-3 rounded-xl bg-gray-800/50 text-white font-medium hover:bg-gray-800 transition">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  <main class="min-h-screen bg-gradient-to-b from-zinc-950 to-black py-12 px-6">
    <div class="container mx-auto max-w-7xl">
      ...
      <!-- Page Header -->
      <div class="mb-10">
        <h1 class="text-4xl md:text-5xl font-black mb-2">All Appointments</h1>
        <p class="text-gray-400 text-lg">Total: <span class="text-blue-400 font-semibold"><?= $total_appointments ?></span> appointments</p>
      </div>

      <!-- Filter Buttons -->
      <div class="mb-8 flex flex-wrap gap-3">
        <a href="?status=all" class="px-6 py-2 rounded-full font-medium transition <?= $filter_status === 'all' ? 'bg-gradient-to-r from-blue-600 to-purple-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700' ?>">
          All
        </a>
        <a href="?status=pending" class="px-6 py-2 rounded-full font-medium transition <?= $filter_status === 'pending' ? 'bg-yellow-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700' ?>">
          Pending
        </a>
        <a href="?status=confirmed" class="px-6 py-2 rounded-full font-medium transition <?= $filter_status === 'confirmed' ? 'bg-green-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700' ?>">
          Confirmed
        </a>
        <a href="?status=completed" class="px-6 py-2 rounded-full font-medium transition <?= $filter_status === 'completed' ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700' ?>">
          Completed
        </a>
        <a href="?status=cancelled" class="px-6 py-2 rounded-full font-medium transition <?= $filter_status === 'cancelled' ? 'bg-red-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700' ?>">
          Cancelled
        </a>
      </div>
      ...
      <div class="bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-700 rounded-3xl overflow-hidden">
            ...
            <table class="w-full">
              <thead class="bg-gray-800/50 border-b border-gray-700">
                <tr class="text-gray-400 text-sm font-semibold uppercase tracking-wider">
                  ...
                  <th class="px-6 py-5 text-center">Booked</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-700">
                <?php foreach ($appointments as $apt): 
                  ...
                  $booked_date = (new DateTime($apt['created_at']))->format('M d, Y g:i A');
                ?>
                  <tr class="hover:bg-gray-800/50 transition">
                    <td class="px-6 py-5 font-medium text-purple-400">#<?= htmlspecialchars($apt['appointment_id']) ?></td>
                    <td class="px-6 py-5 font-medium"><?= htmlspecialchars($apt['customer_first_name'] . ' ' . $apt['customer_last_name']) ?></td>
                    <td class="px-6 py-5 text-gray-300 text-sm">
                      <div><?= htmlspecialchars($apt['customer_email']) ?></div>
                      <div><?= htmlspecialchars($apt['customer_phone'] ?? 'N/A') ?></div>
                    </td>
                    <td class="px-6 py-5 text-gray-300"><?= htmlspecialchars($apt['barber_first_name'] . ' ' . $apt['barber_last_name']) ?></td>
                    <td class="px-6 py-5 text-gray-300"><?= htmlspecialchars($apt['service_name']) ?></td>
                    <td class="px-6 py-5 text-gray-300">
                      <div><?= htmlspecialchars((new DateTime($apt['appointment_date']))->format('M d, Y')) ?></div>
                      <div class="text-sm text-gray-400"><?= htmlspecialchars(date('g:i A', strtotime($apt['appointment_time']))) ?></div>
                    </td>
                    <td class="px-6 py-5 font-medium">₱<?= htmlspecialchars(number_format($apt['price'], 2)) ?></td>
                    <td class="px-6 py-5 text-center">
                      <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold <?= $color ?>">
                        <?= ucfirst($apt['status']) ?>
                      </span>
                    </td>
                    <td class="px-6 py-5 text-center text-gray-400 text-sm"><?= htmlspecialchars($booked_date) ?></td>
                  </tr>
                <?php endforeach; ?>
              </tbody>
            </table>
        ...
        <?php else: ?>
          <div class="p-12 text-center">
            ...
            <p class="text-gray-400 text-lg">No appointments found</p>
  ...
  <script>
    ...
    // Close sidebar and logout modal when pressing Escape key
